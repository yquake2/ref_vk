...
CreateDescriptorPool(void)
{
	VkDescriptorPoolSize poolSizes[] = {
		...
		// sampler
		{
			...
			.descriptorCount = MAX_VKTEXTURES + MAX_LIGHTMAPS * 2 + 3
		}
	};
...
void
QVk_DrawColorRect(float *ubo, VkDeviceSize uboSize, qvkrenderpasstype_t rpType)
{
	uint32_t uboOffset;
...
QVk_DrawColoredTexRect(const float *ubo, VkDeviceSize uboSize, qvktexture_t *texture, const float colorTint[3])
{
	...
	struct {
		...
		._pad = {0}, // explicit zero to avoid pushing uninitialised bytes
...
#define ERRSTR(r) case VK_ ##r: return "VK_"#r
	switch (errorCode)
	{
		...
		ERRSTR(PIPELINE_COMPILE_REQUIRED);
